...
async function verifyChain(chain) {
  for (let i = 0; i < chain.length; i++) {
    const b = chain[i];
    const expected = await sha256(`${b.index}|${b.timestamp}|${b.action}|${b.content}|${b.author}|${b.previousHash}`);
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hp-head { padding: 22px 24px; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; position: sticky; top:0; background: var(--s1); }
  ...
  .block-dot { position: absolute; left: 0; top: 5px; width: 26px; height: 26px; border-radius: 50%; border: 2px solid; display: flex; align-items: center; justify-content: center; font-size: 10px; background: var(--s1); flex-shrink: 0; }
...
export default function ChainDoc() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (hash) {
        const d = await store.get(`doc:${hash}`);
        if (d) {
  ...
  const refreshList = async () => {
    const keys = await store.list("doc:");
    ...
    setDocs(list.map((d) => ({ id: d.id, title: d.title, lastModified: d.lastModified, blocks: d.chain.length })));
  };
  ...
  const saveContent = async () => {
    ...
    const ok = await store.set(`doc:${updated.id}`, updated);
  ...
  const openDoc = async (id) => {
    const d = await store.get(`doc:${id}`);
      ...
      <div className="app">
        ...
        <div className="home">
          ...
          <p className="brand-sub">
            Documentos con <strong>registro inalterable</strong>. Cada edición y firma queda sellada en una cadena criptográfica SHA-256 — cualquier modificación no autorizada es detectable al instante.
          </p>

          <div className="create-box">
            <input
              className="input-main"
              placeholder="Tu nombre completo (quedará registrado como autor)"
              value={author}
          ...
          <div>
            ...
            {docs.length === 0 ? (
              <div className="empty">
                ...
                Escribe tu nombre y crea el primero.
              </div>
            ) : (
              docs.map((d) => (
                <div key={d.id} className="doc-row" onClick={() => openDoc(d.id)}>
                  ...
                  <span className="badge">{d.blocks} bloque{d.blocks !== 1 ? "s" : ""}</span>
                </div>
              ))
            )}
          </div>
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="toolbar">
          <div className="tb-brand" onClick={goHome} title="Volver al inicio">
            <div className="tb-hex" />
            <span className="tb-name">ChainDoc</span>
          </div>
          <div className="sep" />
          <input
            className="title-inp"
          ...
          />
          <button className="tb-btn" onClick={handleVerify} title="Verificar integridad criptográfica">⬡ Verificar</button>
                    ...
                    <div key={i} className="sig-slot">
                      ...
                      <div className="sig-row">
                        <input
                          ...
                          value={s}
                          onChange={(e) => {
                            const ns = [...signers]; ns[i] = e.target.value; setSigners(ns);
                          }}
                ...
                {[...doc.chain].reverse().map((b) => {
                  ...
                  return (
                    <div key={b.index} className="block-entry">
                      <div className="block-dot" style={{ borderColor: meta.color, color: meta.color }}>
                        {meta.icon}
                      </div>
                      <div className="block-card" style={{ borderColor: meta.border }}>
                        <div className="bc-top">
                          <span className="bc-action" style={{ color: meta.color, background: meta.bg }}>
                            {b.action}
                          </span>
